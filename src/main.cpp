...
//typedef std::vector<neighborT> threadneighborsT; // A list of pairs that should be computed by each thread
...
void MDSimulation::run(int nsteps, double dt) {
    ...
    for (int istep = 0; istep < nsteps; ++istep) {
        ...
        for (neighborsT::const_iterator pair = neighbors.begin(); 
             pair != neighbors.end(); 
             pair++) {
    ...
    std::cout << "Total runtime:        " << omp_get_wtime() - start_time << std::endl;
    std::cout << "   Neighborlist time: " << neighborlist_time << std::endl;
    std::cout << "   Forces time:       " << forces_time << std::endl;
}
